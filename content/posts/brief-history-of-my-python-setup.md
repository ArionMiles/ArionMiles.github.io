...
**Note:** This isn't meant to be a comprehensive article comparing pros and cons of each tool used here, or even meant to give you a brief look into them. It's a simple log of my experience with these tools.
...
This module is drop-in replacement for the `venv` module in the py3 stdlib and offers more features as well, but I never used them as my needs were very limited.

### venv

I don't even remember when I started using this. I guess one day I just started using the built-in functionality instead of having to install `virtualenv`, and my needs weren't complex, and both of them satisfied them adequately. I still use this to this day. Every project gets its separate virtual environment, generally all of them are named `.venv`.

### pipenv

This is really touted as the best possible solution for python's bad tooling. But it has its issues. It offers the lockfile goodness, but it didn't sit right with me. The reason I stopped using pipenv was that an upgrade to pip broke it once, and it left a bad taste in my mouth.

Today, pipenv is the official recommendation from Python. I might get around someday to trying it out again. But not anytime soon.

### docker

The only reason I first started using docker was because a project I was working on used Redis, and since Redis doesn't exactly work on Windows, I had to use containers. I had a vagrant box with Ubuntu 16.04 which had some version of docker in it, I don't even remember which.

Containers were fun, unless you were me, back in 2018, on a slow internet connection. Then it sucked waiting for the all the different images to be done downloading, and then waiting for my container image being built. If I updated my `requirements.txt` file, I'd have to recreate my image and you can't take advantage of cached pip packages in this scenario, so it'd download all of them all over again. Not to mention, starting up and shutting down the vagrant box took its own sweet time.
...
I first came across conda through a workshop at PyCon. I never really gave it a chance back then, as I was comfortable with venv, and never really understood the importance of it until recently. Conda is the default package manager with Anaconda, which comes with a bunch of datascience and machine learning packages pre-installed, so I'd always associate it with those usecases.
...
In my recent project I did make extensive use of conda though, and learnt how much more it has to offer. It's important to understand that conda exists outside of python. And so, it can be used for isolating environments for any programming language. The isolation it offers is on a whole different level than what pip or any other tools do. You can isolate system level dependencies, e.g. the dependencies you install using the distro's package manager. This is why conda can be used to manage different versions of pythons as well. Conda environments can be easily exported to `environment.yml` files and then recreated exactly in a different machine.

Of course, this means you must install the packages through conda's repositories (conda forge), and if they don't exist there, you'll have to either create your own recipe and put it up there or deal with it. Though, chances of not finding your package on there would be rare, so it's a good bet. If your python packages aren't available through conda forge, you can specify them in your `environment.yml` file as a [pip entry](https://conda.io/docs/user-guide/tasks/manage-environments.html#create-env-file-manually).
...
This immediately became my favorite python tool ever! Python doesn't get tooling right, so it's a breathe of fresh air when you find something that works like pyenv. It works same as py launcher, but for Linux. You can set a global python version here as well.

You can also set python versions per a particular project directory by running `pyenv local <versionNumber>` and a `.python-version` file is created in that directory. Then pyenv will automatically run all files in it using the version number specified in the file.

And it does more! You can literally install any implementation of python using pyenv. Wanna see how much more performant your code is in `pypy`? You can install a pypy implementation. Wanna run python in a JVM? `jython` is available too! You can download the latest stable, dev or release candidate versions of CPython and a lot more! You can even install anaconda or miniconda with specific python versions using pyenv.
...
I'll wholeheartedly support PyCharm for Django projects, as the IntelliSense support on it for Django is better than VSCode's. It can also interface with databases really well, and that's a plus when you're working on a Django project. They've also added support for recognizing WSL's virtual environments.
...
Instead of doing that, I'd say leave default python alone, install a python version with pyenv and set it as global.
